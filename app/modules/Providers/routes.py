...
async def create_provider(provider_data: ProviderBase, current_user=Depends(get_current_user), session: AsyncSession = Depends(get_session)):
...
async def get_provider_by_user_id(current_user=Depends(get_current_user), session: AsyncSession = Depends(get_session)):
...
async def update_provider(provider_id: str, provider_data: ProviderBase, session: AsyncSession = Depends(get_session)):
